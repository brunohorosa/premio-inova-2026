...
A Polícia Civil de MG validou a solução como órgão piloto externo no que tange a descentralização de cotas orçamentárias, além de mais doze órgãos manifestaram interesse formal em adotá-la. A iniciativa usa software livre, versionamento Git e log estruturado por operação — governança formal desde a base.
```

### Resumo para votação popular `[máx 486 caracteres — atual: 363]`
```
Imagine um servidor público que passa horas digitando no sistema financeiro do Estado dados que já estão em planilhas. A SEPLAG criou uma automação que faz esse trabalho em segundos: 50 operações do processo de alteração orçamentária que levavam 1h17min agora levam 28 segundos, sem erros. Já processou R$ 1,72 bilhão. Com isso, os servidores ficam livres para o que importa: atender melhor o cidadão.
```
...
```
Consumo de horas com digitação manual no sistema operacional
Erros de digitação em operações orçamentárias críticas
Mais seguro e estável comparado com automações RPA visual como Power Automate
Execução sem bloqueio do computador como acontece com PowerAutomate
Maior rastreabilidade dos processos executados
```

### Público-alvo
```
Equipe central de orçamento do Estado
Equipes de orçamento dos órgãos finalísticos (descentralização)
...
3 fluxos já estruturados: cota, crédito, descentralização
...
### Valor gerado
```
redução no tempo de execução comparado com manualmente e PowerAutomate
liberação de servidores para atividades de maior valor agregado
...
13 órgãos: expansão e validação
```

### Detalhamento da solução, aprimoramento e multiplicação
```
Expandir para as áreas centrais de orçamento do Estado
Atender os 13 órgãos finalistícos com interesse já manifestado
...
```
A iniciativa consiste em uma biblioteca Python que dialoga com o terminal TN3270 da PRODEMGE via protocolo nativo, desenvolvida e implantada na SPLOR/SEPLAG com três fluxos completos produzidos.
...
Em escala de produção, os números falam por si: somente no fluxo de remanejamento, em menos de um mês (19/05 a 16/06/2026), foram gerados 109 documentos SIAFI, processadas 257 alterações orçamentárias e alocados cerca de R$ 1,72 bilhão, atendendo 29 unidades orçamentárias, tudo sem erros. Todos esses docuemntos gerados foram processados e publicados via decreto orçamentário, ressaltando a importância de se minimizar erros de digitação.
...
```
A replicabilidade foi validada empiricamente: o segundo fluxo (alteração orçamentária) foi construído a partir do primeiro (cota) com esforço significativamente menor, reutilizando as funções de login, navegação e finalização sem qualquer modificação.
...
Os outros doze órgãos com interesse formal (PMMG, CBMMG, GMG, SEJUSP, FES, ARMBH, SECULT, SEINFRA, DER, Fapemig e SEPLAG) são parceiros de expansão, cada um que adota contribui para validar novos contextos e, potencialmente, para ampliar o catálogo de fluxos disponíveis a todos.

O programa Automatiza.MG pode se tornar a ser um parceiro estratégico de disseminação, tendo em vista que a plataforma complementa o programa e pode aproveitar sua estrutura institucional para alcançar mais órgãos com menos esforço de divulgação.
```
